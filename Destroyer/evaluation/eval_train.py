...
def val_train_eval(cfg, train_scores, generator, flownet, segnet, autoencoder, iter):
    ...
    gt = gt_loader()
    assert len(a_sse_group) == len(gt), f'Ground truth has {len(gt)} videos, but got {len(a_sse_group)} detected videos.'
    ...
    for net, sse_group in enumerate([g_sse_group, a_sse_group]):
        ...
        if net == 'Generator':
            ...
            train_scores['a_auc_list'].append(best_auc)
            a_auc = best_auc 

            # AUC
            save_text(f"[{net}][{iter}] AUC: {best_auc}, weight: {best_weight}", f'results/{dataset_name}/psnrs-auc.txt')
            save_auc_graph_train(iters=train_scores['iter_list'], scores=train_scores['a_auc_list'], file_path=f'results/{dataset_name}/a_auc_itr_graph.jpg')

    return -1, g_auc, a_auc, train_scores